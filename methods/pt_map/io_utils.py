...
def parse_args(script):
    ...
    parser.add_argument('--train_aug', action='store_true',
                        help='perform data augmentation or not during training ')  # still required for
    ...
    if script == 'train':
        ...
        parser.add_argument('--stop_epoch', default=400, type=int,
                            help='Stopping epoch')  # for meta-learning methods, each epoch contains 100 episodes.
        ...
        parser.add_argument('--model_dir', type=str, help='the pretrained model path ')
        parser.add_argument('--file_name', type=str, help='where the features will be saved ')
        parser.add_argument('--json_dir', type=str, default='./', help='')
